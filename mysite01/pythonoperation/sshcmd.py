...
def copy_rsa(ip, username, password):
    ...
    try:
        ...
        stdin, stdout, stderr = conn.exec_command(
            "echo '{0}'>>/root/.ssh/authorized_keys;echo '{1}'>>/root/.ssh/authorized_keys".format(rsa[0], rsa[1]))
        result = stdout.readlines()
        ret = ''.join(result)
    except:
        print
        ...
        if args.listhost is not None and args.filehost is None:
            if args.command is not None:
                ...
                for ip in args.listhost.split(','):
                    ...
                    G('-' * 80)
                    print
                    sftp_auto(ip, username, password)
                    print
